        ...
        return (
            <Controller
                ...
                render={({field}) => {
                    return (
                        <Select
                            className={`mb-2  appearance-none border rounded w-full text-gray-700 leading-tight focus:outline-none focus:shadow-outline ${className}`}
                            options={options}
                            ...
                            ref={selectRef}
                            value={field.value && {
                                label: options?.filter((option: any) => option.label === JSON.parse(field.value).name)[0]?.label,
                                value: field.value
                            }}
                            {...props}
                        />
